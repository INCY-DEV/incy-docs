...
| `banner-button-text` | — | текст | — | Текст кнопки баннера |
...
Баннер показывается, только когда выполнены **оба** условия:

1. Провайдер — **премиум** (активная премиум-подписка).
2. Баннер **включён в премиум-панели** (`bannerEnabled`).

Сами заголовки баннер НЕ включают — это делается только в панели. Заголовки лишь задают/переопределяют содержимое.
...
| `banner-button-text` | текст | `bannerButtonText` |
...
>
> Если после резолва `banner-text` пуст — баннер не показывается, даже когда он включён в панели.
>
